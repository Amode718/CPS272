...
// 07/20/2022
// Machine Problem 4
// Hashtable authenticating users with username and password
...
void List<T>::remove() throw (logic_error)
{
	...
	else                              // Remove last item
	{
		...
		for (q = head; q->next != cursor; q = q->next)
			;
		q->next = 0;
		cursor = head;			// Shouldn't this point to q, instead?
	}
...
bool HashTbl<T, KF>::retrieve(KF searchKey, T &dataItem)
{
	...
	do
	{
		if (dataTable[index].getCursor().getKey() == searchKey)
		{
			T temp = dataTable[index].getCursor();
			if(temp.password==dataItem.password)
			return true;
			return false;
		}
	} while (dataTable[index].gotoNext());
