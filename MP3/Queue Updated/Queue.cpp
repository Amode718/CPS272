...
{
	if ((getTimeElapsed()  - serviceLine.front().waitStartTime) > maxWaitTime)
	{
		maxWaitTime = static_cast <double> ((getTimeElapsed()  - serviceLine.front().waitStartTime)) / minuteLength;
	}
